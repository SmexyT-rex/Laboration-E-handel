...
el.innerHTML = `       <div class="box">         <img src="${p.image}" alt="${p.title}" />       </div>       <div class="product-row">         <div class="text-column">           <h3>${p.title}</h3>           <p class="price">${(Number(p.price) * 10).toFixed(2)}kr</p>         </div>         <button class="js-add-to-cart" data-productid="${p.id}">           <i class="bi bi-cart-plus"></i>         </button>       </div>
    `;
...
addToCart(id);
renderCart(); // uppdatera mini-cart direkt
});
});
}
...
const filteredProducts = productArray.filter(p => p.category === category.json);
